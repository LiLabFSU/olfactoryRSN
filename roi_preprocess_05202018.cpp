/*********************************************************************
 *
 * This code is designed for preprocessing ROI timeseries and is based
 * on the open access data from humanconnectome.org. This code does
 * not perform motion correction, detrending, or any of the other 
 * preprocessing steps carried out by the HCP team.
 * 
 * 
 * 
 * 1. update_goodvoxels
 ...
 * 		on local coefficient of variation (inverse of SNR, std/mean).
 * 		This file is useful because it removes voxels with a low SNR,
 * 		subject to the surrounding voxels SNR. This provides similar  
 * 		voxel removal rates in areas of signal dropout as well as in
 * 		areas of normal BOLD signal. Since the olfactory regions are
 * 		located in areas of high signal dropout, this method is
 * 		particularly useful for our interests. HOWEVER, some voxels
 * 		are still included that have negative values for certain TRs.
 * 		Thus this function searches for remaining negative values and
 * 		provides an updated mask with a "0" indicating the voxels to
 * 		be excluded.
 * 
 ...
 *		It also returns the nuimber of TRs in the data file.
 * 
 ...
 * 
 * g++ roi_preprocess_0520201.cpp -o roi_preprocess_05202018
 * 
 * Run:
 * ./a.out rfmri.nii atlas.nii goodvoxels.nii
 *
 * Written by Thomas Campbell Arnold, FSU 2016
 *
 *********************************************************************/
...
// PROTOTYPES
int * update_goodvoxels(char *data_file, char *goodv_file, char *basal_file);
...
int main(int argc, char *argv[]) 
{
	...
	char *data_file, *atlas_file, *goodv_file, *basal_file;
	...
	// call functions
	int *goodv_update = update_goodvoxels(data_file, goodv_file, basal_file);
...
int * update_goodvoxels(char *data_file, char *goodv_file, char *basal_file)
{
	...
	nifti_1_header basal_hdr;
	FILE *f;
	int ret, i, t, neg_data, neg_goodv;
	MY_DATATYPE *data=NULL, *goodv_data=NULL, *basal_data=NULL;
	...
	//float goodv_data[goodv_hdr.dim[1]*goodv_hdr.dim[2]*goodv_hdr.dim[3]];
	//memset(goodv_data,0,goodv_hdr.dim[1]*goodv_hdr.dim[2]*goodv_hdr.dim[3]*sizeof(float));
	...
	// LOAD IN BASAL GANGLION MASK
	
	// open hdr file
	f = fopen(basal_file,"r");
	if (f == NULL) {
        fprintf(stderr, "\nError opening header file %s\n",basal_file);
        exit(1);
    }
    ...
    // read hdr information
    ret = fread(&basal_hdr, MIN_HEADER_SIZE, 1, f);
    if (ret != 1) {
        fprintf(stderr, "\nError reading header file %s\n",basal_file);
        exit(1);
    }
    ...
    // print hdr information to screen
    fprintf(stderr, "\n%s header information:",basal_file);
    fprintf(stderr, "\nXYZT dimensions: %d %d %d %d",basal_hdr.dim[1],basal_hdr.dim[2],basal_hdr.dim[3],basal_hdr.dim[4]);
    fprintf(stderr, "\nDatatype code and bits/pixel: %d %d",basal_hdr.datatype,basal_hdr.bitpix);
    fprintf(stderr, "\nScaling slope and intercept: %.6f %.6f",basal_hdr.scl_slope,basal_hdr.scl_inter);
    fprintf(stderr, "\nByte offset to data in datafile: %ld",(long)(basal_hdr.vox_offset));
    fprintf(stderr, "\n");
	...
	ret = fseek(f, (long)(basal_hdr.vox_offset), SEEK_SET);
	...
	// allocate space to hold the data
	basal_data = (MY_DATATYPE *) malloc(sizeof(MY_DATATYPE) * basal_hdr.dim[1]*basal_hdr.dim[2]*basal_hdr.dim[3]);
	if (basal_data == NULL) {
		fprintf(stderr, "\nError allocating data buffer for %s\n",basal_file);
		exit(1);
	}
	
	ret = fread(basal_data, sizeof(MY_DATATYPE), basal_hdr.dim[1]*basal_hdr.dim[2]*basal_hdr.dim[3], f);
	if (ret != basal_hdr.dim[1]*basal_hdr.dim[2]*basal_hdr.dim[3]) {
		fprintf(stderr, "\nError reading volume 1 from %s (%d)\n",basal_data,ret);
		exit(1);
	}
	...
	// allocate memory for holding mean
	//float data[hdr.dim[1]*hdr.dim[2]*hdr.dim[3]];
	//memset(data,0,hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*sizeof(float));
	...
	for(t=1; t<=hdr.dim[4]; t++){
		...
		for (i=0; i<hdr.dim[1]*hdr.dim[2]*hdr.dim[3]; i++)
		{
			...
			/* REMOVED: removed any voxel that had a value below zero (not valid after DPARSFA)
	...
	// convert to int
	static int goodv_new[1000000];
	// static does not work becuase it is defined by a variable, this should be changed.
...
int check_snr(char *data_file)
{
	...
	//float mean_data[hdr.dim[1]*hdr.dim[2]*hdr.dim[3]];
	...
	// allocate memory for holding mean
	//float var_data[hdr.dim[1]*hdr.dim[2]*hdr.dim[3]];
	//memset(var_data,0,hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*sizeof(float));
...
int roi_extract(char *data_file, char *atlas_file, int *goodv_update){
	...
	**********************************************************************/
	...
	//int roi_array[aal_hdr.dim[1]*aal_hdr.dim[2]*aal_hdr.dim[3]];
	...
	**********************************************************************/
	...
	//float mean_intensity[hdr.dim[1]*hdr.dim[2]*hdr.dim[3]];
	//memset(mean_intensity,0,hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*sizeof(float));
	...
	/*float fmri_array[hdr.dim[1]*hdr.dim[2]*hdr.dim[3]];
	memset(fmri_array,0,hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*sizeof(float));
	for (i=0; i<hdr.dim[1]*hdr.dim[2]*hdr.dim[3]; i++)
	{
		fmri_array[i] = data[i];
	}*/
...
int meancenter_col(int max, int trs){
	 ...
	 * output from nifti_simu.c is in 1200x605 (the last column is zeros). This
...
int voxel_summary(int max, char* data_file, char *atlas_file, char *goodv_file, int *goodv_update){
	...
	/*** convert from float to int ***/
	//int goodv_array[goodv_hdr.dim[1]*goodv_hdr.dim[2]*goodv_hdr.dim[3]];
